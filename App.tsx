...
function smartTitleCase(str: string) {
  if (!str) return ''
  const smallWords = ['and','or','the','a','an','in','on','for','of','to','at','by','with','from']
  const words = str.toLowerCase().split(/\s+/)
  return words.map((w, i) => {
    if (i === 0 || i === words.length - 1 || !smallWords.includes(w)) {
            ...
            <nav className="space-y-4">
              ...
              <div className="space-y-2">
                {current?.sections.map(sec => {
                  ...
                  return (
                    <div key={smartTitleCase(sec.section)}>
                      <div className="text-sm font-semibold text-ink dark:text-white">{smartTitleCase(sec.section)}</div>
              ...
              {current?.sections.map((s) => {
                ...
                return (
                  <section key={s.section} id={`sec-${tabSlug}-${slugify(s.section)}`} className="space-y-4">
                    <h2 className="text-xl font-semibold text-ink dark:text-white">{smartTitleCase(s.section)}</h2>
                    {s.categories.map((c) => {
                      ...
                      return (
                        <div key={c.category} id={anchorId} className="space-y-3">
                          <h3 className="text-sm text-slate-500 dark:text-slate-400">{smartTitleCase(c.category)}</h3>
                          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
                            {c.prompts.map((p, i) => (
